...
<div <?= session()->get('password_reset')==1 || session()->getFlashdata('pass_error') || session()->getFlashdata('pass_confirm_error') ? 
       ' class="modal fade show" id="password_modal" tabindex="-1" aria-modal="true" role="dialog"  style="display: block;" ' :
       ' class="modal fade" id="password_modal" tabindex="-1" aria-hidden="true" '; ?> >
        ...
        <div class="modal-content">
            ...
            <form method="post" role="form" name="password_form" enctype="multipart/form-data" action="<?php echo site_url('profile/update_password'); //  ?>">
                    <div class="modal-body">
                            ...
                            <div class="form-floating mb-3">
                                <?php if (session()->getFlashdata('pass_error')){ ?>
                                    <span class="error-message mb-3"><i class="fas fa-triangle-exclamation"></i> 
                                        <?= session()->getFlashdata('pass_error'); ?>
                                    </span>
                                <?php } ?>
                            ...
                            <div class="form-floating mb-3">
                                <?php if (session()->getFlashdata('pass_confirm_error')){ ?>
                                    <span class="error-message mb-3"><i class="fas fa-triangle-exclamation"></i> 
                                        <?= session()->getFlashdata('pass_confirm_error'); ?>
                                    </span>
                                <?php } ?>
                    ...
                    <div class="modal-footer">
                        <?php if(session()->get('password_reset')==1){ } else { ?>
...
<?= session()->get('password_reset')==1 || session()->getFlashdata('pass_error') || session()->getFlashdata('pass_confirm_error') ? '<div class="modal-backdrop fade show"></div>' : '' ?>
